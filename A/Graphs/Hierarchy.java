...
public class Hierarchy {
    ...
    static boolean[] visited = new boolean[100072];
    static ArrayList<Integer> ans = new ArrayList<>();
    static int[] Parent = new int[100072];
    static ArrayList<ArrayList<Integer>> graph = new ArrayList<>();

    private static void hierarchy(int i) {
        visited[i] = true;
        for (int j = 0; j < graph.get(i).size(); j++) {
            int s = graph.get(i).get(j)-1;
            if (!visited[s]) {
                hierarchy(s);
            }
        }
