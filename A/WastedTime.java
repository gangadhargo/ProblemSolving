...
public class WastedTime {
    ...
    public static void main(String[] args) {
        int n = sc.nextInt();
        int k = sc.nextInt();
        double sumOfPolygonsDistance = 0;
        ...
        for (int i = 1; i<n; i++){
            sumOfPolygonsDistance+=Math.pow(Math.pow(arr[i][0] - arr[i-1][0], 2) + Math.pow(arr[i][1] - arr[i-1][1], 2), 0.5);
        }
        double ans = sumOfPolygonsDistance* k/50.0;
