...
public class ColorfulField {
    ...
    static String[] corps = new String[]{"Carrots", "Kiwis", "Grapes"};
    private static void colorfulField(int[][] waste, int n, int m){
        int a = sc.nextInt();
        int b = sc.nextInt();
        int w = 0;
        for (int i=0; i< waste.length; i++){
            if(waste[i][0] == a && waste[i][1] == b){
                System.out.println("Waste");
                return;
            }
            if(a > waste[i][0] || (a == waste[i][0] && b>=waste[i][1])){
                w++;
            }
        }
        int cell = (a-1)*m + b - w -1;
        int rem = cell%3;
        System.out.println(corps[rem]);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < k; i++) {
            waste[i][0] = sc.nextInt();
            waste[i][1] = sc.nextInt();
        }
        while (t>0){
            colorfulField(waste, n, m);
            t--;
        }
    }
