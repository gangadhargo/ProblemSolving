...
public class GrowingMushrooms {
    ...
    private static void sortMapByValue(HashMap<Integer, Double> map) {
        List<Map.Entry<Integer, Double>> list = new LinkedList<>(map.entrySet());
        Collections.sort(list, new Comparator<Map.Entry<Integer, Double>>() {
            @Override
            public int compare(Map.Entry<Integer, Double> o1, Map.Entry<Integer, Double> o2) {
                return o1.getValue().compareTo(o2.getValue()) == 0 ? o1.getKey() - o2.getKey() : o2.getValue().compareTo(o1.getValue());
            }
        });
        for (int i=0; i<list.size(); i++) {
            Map.Entry<Integer, Double> sortedMap = list.get(i);
            System.out.println(sortedMap.getKey() + " " + String.format("%.2f", sortedMap.getValue()));
        }
    }
    ...
    public static void main(String[] args) {
        ...
        int t2 = sc.nextInt();
        int s = sc.nextInt();
        Double k = (Double) ((100 - s)/100.0);
        HashMap<Integer, Double> map = new HashMap();

        for (int i = 0; i < n; i++) {
            int a = sc.nextInt();
            int b = sc.nextInt();
            Double sum1 = (a * t1 * k) + (b * t2);
            Double sum2 = (b * t1 * k) + (a * t2);
            map.put(i + 1, Math.max(sum1, sum2));
        }
        sortMapByValue(map);
    }
