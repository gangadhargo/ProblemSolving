...
public class PresidentsOffice {
    ...
    public static void main(String[] args) {
        ...
        int m = sc.nextInt();
        String p = sc.next();
        char[][] desks = new char[n][m];
        for (int i = 0; i < n; i++) {
            String s = sc.next();
            for (int j = 0; j < s.length(); j++) {
                desks[i][j] = s.charAt(j);
            }
        }
        HashSet<Character> set = new HashSet<>();
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (desks[i][j] == p.charAt(0)) {
                    if(i+1< n && desks[i+1][j]!='.' && desks[i+1][j]!=p.charAt(0)){
                        set.add(desks[i+1][j]);
                    }
                    if(j+1< m && desks[i][j+1]!='.' && desks[i][j+1]!=p.charAt(0)){
                        set.add(desks[i][j+1]);
                    }
                    if(i-1>= 0 && desks[i-1][j]!='.' && desks[i-1][j]!=p.charAt(0)){
                        set.add(desks[i-1][j]);
                    }
                    if(j-1>= 0 && desks[i][j-1]!='.' && desks[i][j-1]!=p.charAt(0)){
                        set.add(desks[i][j-1]);
                    }
                }
            }
