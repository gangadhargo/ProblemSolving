...
public class TowerOfHanoi {
    ...
    public static void main(String[] args) {
        ...
        int ans = tower(n, 1, 2, 3);
        System.out.println(b == true ? ans : -1);
    }

    private static int tower(int n, int from, int to, int temp) {
        if(n == 0) return 0;
        if(a[n-1] == from){
            return tower(n-1, from, temp, to);
        } else if (a[n-1] == to){
            return (1 << (n-1))+tower(n-1, temp,to, from);
        }
            b = true;
            return 1;
    }
