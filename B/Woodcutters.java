...
public class Woodcutters {
    ...
    public static void main(String[] args) {
        int n = sc.nextInt();
        int[][] heigths = new int[n][n];
        int[][] dp = new int[n][3];
        for (int i = 0; i < n; i++) {
            heigths[i][0] = sc.nextInt();
            heigths[i][1] = sc.nextInt();
        }
       // dp[i][0] means tree fell to left count
        // dp[i][1] means tree is still and cant cut down
        //dp[i][2] means tree fell to right count

        dp[0][0] = 1;
        dp[0][1] = 0;
        if ((heigths[0][0] + heigths[0][1]) < heigths[1][0]) {
            dp[0][2] = 1;
        } else {
            dp[0][2] = 0;
        }
        for (int i = 1; i < n; i++) {
            ...
            if (heigths[i][0] - heigths[i][1] > heigths[i - 1][0]) {
                dp[i][0] = Math.max(dp[i - 1][0], dp[i - 1][1]) + 1;
            }
            if (heigths[i][0] - heigths[i][1] > heigths[i - 1][0] + heigths[i - 1][1]) {
                dp[i][0] = Math.max(dp[i][0], dp[i - 1][2]) + 1;
            }
            if (i == n - 1 || (heigths[i][0] + heigths[i][1] < heigths[i + 1][0])) {
                dp[i][2] = dp[i][1] + 1;
            }
        }
