...
public class AgoodString {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < t; i++) {
            ...
            System.out.println(goodString(s, 'a'));
        }
    }

    public static int goodString(String s, char c) {
        if (s.length() == 1) {
        ...
        for (int i = 0; i < n; i++) {
            if (i < n / 2) {
                if (s.charAt(i) == c) ones++;
            } else {
                if (s.charAt(i) == c) twos++;
            }
        }
        c++;
        int left = goodString(s.substring(0, n / 2), c);
        int right = goodString(s.substring(n / 2, n), c);
        int ans = Math.min((n / 2) - ones + right, (n / 2) - twos + left);
        return ans;
    }
